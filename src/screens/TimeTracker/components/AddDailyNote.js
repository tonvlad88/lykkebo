...
// Global imports
import { convertTimeToSeconds, getTimeInterval, toTimestamp } from '../../../services/common';
...
class AddDailyNote extends Component {
  ...
  async componentDidMount() {
    ...
    const total = getTimeInterval(startTime, endTime, convertTimeToSeconds(breakTime));
    console.log('selectedBooking', this.props.selectedBooking)
    this.setState({
      total,
      selectedBooking: this.props.selectedBooking === undefined || this.props.selectedBooking.length == 0 ? {} : this.props.selectedBooking,
      clientID: this.props.selectedBooking === undefined || this.props.selectedBooking.length == 0 ? 0 : this.props.selectedBooking[0].customer_id,
  ...
  render() {
    const { selectedDate, selectedDateFormatted, userId, internalOnly, bookings } = this.props;
    const {
      startTime, endTime, total, comment, loaded, breakTime, selectedBooking
    } = this.state;
      ...
      <View style={styles.container}>
        <ScrollView>
          ...
          <Row style={{
            ...
            <Col>
              <Text style={{fontSize: 20, fontWeight: 'bold', textAlign: 'center'}}>{new XDate(selectedDate).toString('dd MMMM, yyyy')}</Text>
            </Col>
          </Row>
              ...
              <Row style={styles.dailyNoteItemContainer}>
                ...
                <Col style={styles.dailyNoteClock}>
                  ...
                  <Dropdown
                    // label={props.label}
                    fontSize={16}
                    value={Object.keys(selectedBooking).length === 0 && selectedBooking.constructor === Object ? '' : selectedBooking[0].customer_id}
                    ...
                    onChangeText={(clientID) => {
                      ...
                      this.setState({clientID: bookingData[0].customer_id, selectedBooking: bookingData})
                    }}
          ...
          <Button
            ...
            onPress={() => {
              const data = {
                ...
                client_id: Object.keys(selectedBooking).length === 0 && selectedBooking.constructor === Object ? '' : selectedBooking[0].customer_id,
                job_id: Object.keys(selectedBooking).length === 0 && selectedBooking.constructor === Object ? '' : selectedBooking[0].job_id,
                comment,
                break: breakTime,
              };
          ...
          <Button
            buttonStyle={{margin: 10, marginTop: 0, height: 46, backgroundColor: '#1C323A'}}
            onPress={() => this.props.closeModal()}
            titleStyle={{fontWeight: 'bold'}}
            title={i18n.t('cancel')} />
        </ScrollView>
