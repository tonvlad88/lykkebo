...
// Actions
import { saveDailyNote, getTracker, deleteNoteTrackerData, updateDailyNote } from '../../../redux/actions/tracker';
...
import { convertSecondsToTime, toTimestampWithSeconds } from '../../../services/common';
...
class TimeTrackerScreen extends React.Component {
  ...
  _setData = () => {
    ...
    AsyncStorage.getItem('user_id').then((userId) => {
      AsyncStorage.getItem('user_relation').then((userRelation) => {
        this.props.getTracker(userId, XDate(selectedDate).toISOString().slice(0, 10)).then((res) => {
          // console.log('res', res)
          this.setState({
            userId,
            userRelation,
        ...
        backgroundColor={data.total_time > 0 ? '#72B565' : 'red'}>
          <List>
            <ListItem
              ...
              <Grid>
                <Col>
                  <Text style={{flex: 1, fontWeight: 'bold'}}>{`BOOKING ${data.job_id}`}</Text>
                </Col>
                {/* <Col>
                  <TouchableOpacity>
                    <MaterialIcons
            ...
            <ListItem style={styles.noMarginLeft} avatar>
              ...
              <Body style={[styles.paddingLeft5, {justifyContent: 'center', borderBottomWidth: 0}]}>
                ...
                <Text note numberOfLines={1} style={{color: 'white'}}>{data.customer_name}</Text>
              </Body>
              <Right style={{borderBottomWidth: 0, justifyContent: 'center', alignItems: 'center' }}>
                <Text style={{fontSize: 17, fontWeight: 'bold', color: 'white'}}>{convertSecondsToTime(data.total_time)}</Text>
              </Right>
            </ListItem>
            {/* <View style={{
    ...
    <Swipeout
              ...
              [
                ...
                },
                {
                  ...
                  onPress: async () => {
                    ...
                    await this.props.getTracker(this.state.userId, XDate(this.state.selectedDate).toISOString().slice(0, 10));
                  },
                },
              ],
              {cancelable: false},
      ...
      backgroundColor="transparent">
        ...
        <ListItem
          ...
          <Grid>
            <Col style={{flex: 1}}>
              <Text style={{flex: 1, fontWeight: 'bold'}}>{`${i18n.t('internalTimeRecording')} ${data.id}`}</Text>
            </Col>
            {/* <Col style={{width: 50}}>
              <Icon
                onPress={() => {
        ...
        <ListItem style={styles.noMarginLeft} avatar>
          <Left />
          <Body style={[styles.paddingLeft5, {justifyContent: 'center', borderBottomWidth: 0}]}>
            <Text style={{fontSize: 10, color: '#BFBEC4'}}>{`Time: ${data.start} - ${data.end}`}</Text>
            <Text style={{fontSize: 10, color: '#BFBEC4'}}>{i18n.t('break')}: {data.break}</Text>
  ...
  openTimeDetails(details) {
    ...
    AsyncStorage.setItem('selectedTimeTrackerDetails', JSON.stringify(details)).then(() => {
      AsyncStorage.setItem('selectedTimeTrackerDetailsDate', JSON.stringify(selectedDate.getTime())).then(() => {
  ...
  render() {
    const {
      navigation, saveDailyNote, getTracker, updateDailyNote,
    } = this.props;
    const { isLoading, trackerBookings, trackerDailyNotes } = this.props.tracker;
    ...
    return (
      <Container>
        ...
        <CustomHeader
          ...
          rightIconName="add"
          onPressRightIcon={() => this.setState({showAddDailyNoteModal: true, internalOnly: false, selectedBooking: []})}
          title={i18n.t('timeTracker')} />
        ...
        <View style={{
          ...
          <View style={styles.calendarTitle}>
            <TouchableOpacity
              ...
              <Text style={{fontWeight: 'bold', fontSize: 20, color: 'white'}}>{XDate(selectedDate).toString('dd MMMM yyyy')}</Text>
              <AntDesign
                  name="calendar"
                  size={30}
                  color="white"
          ...
          <View style={styles.calendarArrow}>
            <TouchableOpacity
              onPress={() => {
                const tempDate = new Date(selectedDate);
                tempDate.setDate(tempDate.getDate() + 1);
                this.setState({selectedDate: XDate(tempDate).toString('yyyy-MM-dd')});
                getTracker(userId, XDate(tempDate).toISOString().slice(0, 10));
              }}>
              <Icon
                name="arrow-forward"
        ...
        <Modal
          isVisible={isOpenNewCalendarModal}>
          <View style={{
            ...
            <TouchableOpacity onPress={() => this.setState({isOpenNewCalendarModal: false})}>
        ...
        <Modal
          isVisible={showAddDailyNoteModal}>
          <AddDailyNote
            ...
            saveDailyNoteProp={async (data) => {
              ...
              await saveDailyNote(data, XDate(selectedDate).toString('yyyy-MM-dd'));
              await getTracker(userId, XDate(selectedDate).toString('yyyy-MM-dd'));
            }}
            closeModal={() => this.setState({showAddDailyNoteModal: false})} />
        </Modal>

        <Modal
          isVisible={showEditDailyNoteModal}>
          <EditDailyNote
            ...
            updateDailyNoteProp={async (data) => {
              ...
              await getTracker(userId, XDate(selectedDate).toISOString().slice(0, 10));
            }}
          ...
          <ScrollView>
            ...
            <View style={styles.addButtonInternalTimeContainer}>
              <Button
                ...
                // success
                onPress={() => {
                  this.setState({showAddDailyNoteModal: true, internalOnly: true, selectedBooking: []});
                }}>
                <Text style={{color: 'white', fontWeight: 'bold'}}>{i18n.t('addInternalTimeRecordingButtonLabel')}</Text>
              </Button>
            </View>
